...
public abstract class AUlNumber<T extends Number> extends Number implements Calculable<AUlNumber<T>>, Comparable<Number>, Cloneable {
    ...
    @SafeVarargs
    public static @NotNull AUlNumber<?> cast(@NotNull Object o, Class<? extends Number>... types) throws Exception {
        if (o instanceof AUlNumber) {
            AUlNumber<?> number = (AUlNumber<?>) o;

            if (!(types.length == 0 || number.value() instanceof AUlNumber || Arrays.asList(types).contains(number.getTypeClass()))) {
