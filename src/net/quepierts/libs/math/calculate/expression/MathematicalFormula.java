...
import net.quepierts.libs.utils.Patterns;
import net.quepierts.libs.utils.StringUtils;

import java.util.*;
import java.util.regex.Matcher;

public class MathematicalFormula extends AUlNumber<Double> {
    private static final List<MathematicalFormula> GLOBAL_FORMULAS = new ArrayList<>();
    ...
    public static MathematicalFormula compile(String expression, boolean global) throws FunctionException {
        expression = analyzeSyntax(expression);

//        System.out.println(expression);

        Matcher function = Patterns.FUNCTION_ENTRY.matcher(expression);
        Matcher lambda = Patterns.LAMBDA_ENTRY.matcher(expression);
        if (function.find()) {
            String name = function.group();
            ...
            if (global) {
                GLOBAL_FORMULAS.add(result);
                AVALIABLE_NAMES.add(result.name);
            }

            return result;
        } else if (lambda.find()) {
            if (global) {
                throw new FunctionException("A Anonymous Function Cannot Be Constant");
            }

            String entry = lambda.group();
            expression = expression.replace(entry, "");

            MathematicalFormula result = new MathematicalFormula(compile(expression), "");

            for (String parameter : StringUtils.extractParameters(entry)) {
                if (parameter.isEmpty()) {
                    continue;
                }
...
//                System.out.println("Value = " + value + ", Replacement = " + last.value);
    ...
    public static MathematicalFormula getStatic(String name, int parameterAmount) {
        ...
        for (MathematicalFormula constant : GLOBAL_FORMULAS) {
