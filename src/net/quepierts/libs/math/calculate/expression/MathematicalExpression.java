...
public abstract class MathematicalExpression {
    ...
    public static void enableLogging() {
        ...
        System.out.println("Compile Logging: Enabled");
    }

    public static void disableLogging() {
        logging = false;
        System.out.println("Compile Logging: Disabled");
    }
    ...
    public static MathematicalExpression valueOf(String expression, @NotNull MathematicalExpression[] values) throws FunctionException {
        ...
        if (logging)
            System.out.println("[Compiler] Start Construct: " + expression + ", Parameters Amount = " + values.length + ", Negative = " + negative);
        ...
        if (logging)
            System.out.println("[Compiler] Expression = " + expression + ", signal = " + signal + ", parameters = " + expressions.size());
    ...
    private static MathematicalExpression construct(String expression, boolean negative, String signal, MathematicalExpression... expressions) throws FunctionException {
        if (logging)
            System.out.println("[Compiler] signal = " + signal + ", expression = " + expression);
        ...
        switch (signal) {
            ...
            default:
                if (expressions.length == 1) {
                    if (negative)
                        expressions[0].negative = !expressions[0].negative;
    ...
    private static MathematicalExpression getFunction(String expression, MathematicalExpression[] parameters) throws FunctionException {
        ...
        if (logging)
            System.out.println("[Compiler] Construct Function " + name + ", By Expression " + expression);
    ...
    private static MathematicalExpression getSingle(String expression) {
        ...
        if (logging)
            System.out.println("[Compiler] Expression = " + expression + ", length = " + expression.length());
